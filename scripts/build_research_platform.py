...
class ResearchPlatformBuilder:
    ...
    def build_platform(self, skip_phases: list = None):
        ...
        if "fetch_data" not in skip_phases:
            result, error = self.run_phase("Phase 1: Data Fetching", fetch_data)
            if error:
        ...
        if "citations" not in skip_phases:
            result, error = self.run_phase(
                "Phase 2: Citation Tracking", generate_citation_report, repos_data
            )
            if error:
        ...
        if "search" not in skip_phases:
            result, error = self.run_phase(
                "Phase 3: Search Indexing", build_search_index, repos_data
            )
            if error:
                errors["search"] = error
            else:
        ...
        if "visualizations" not in skip_phases:
            result, error = self.run_phase(
                "Phase 4: Visualizations", generate_all_visualizations, repos_data, citation_report
            )
            if error:
        ...
        if "community" not in skip_phases:
            result, error = self.run_phase(
                "Phase 5: Community Features", generate_reproducibility_report, repos_data
            )
            if error:
        ...
        if "code_quality" not in skip_phases:
            result, error = self.run_phase(
                "Phase 6: Code Quality Analysis", analyze_all_repositories, repos_data
            )
            if error:
...
def main():
    ...
    parser.add_argument("org_name", nargs="?", help="GitHub organization name")
    parser.add_argument(
        "--skip",
        nargs="+",
        choices=[
