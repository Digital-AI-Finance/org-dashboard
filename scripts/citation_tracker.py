...
class CitationGraph:
    ...
    def build_graph(self, repos_data: List[Dict]) -> Dict[str, Any]:
        ...
        graph_data = {
            ...
            'network': {
                'nodes': [
                    {'id': name, 'type': data['type'], 'citations': self.citation_counts.get(name, 0)}
    ...
    def calculate_impact_metrics(self) -> Dict[str, Any]:
        ...
        for repo_name, node in self.nodes.items():
            ...
            metrics[repo_name] = {
                ...
                'citations_per_paper': pub_citations / publications_count if publications_count > 0 else 0
            }
    ...
    def get_citation_network_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'most_citing_repos': [{'repo': name, 'citations': count} for name, count in most_citing],
            'average_citations_per_repo': sum(self.citation_counts.values()) / len(self.nodes) if self.nodes else 0,
    ...
    def _calculate_h_index(publications: List[Dict]) -> int:
        ...
        citations = sorted(
            [pub.get('citation_count', 0) for pub in publications],
            reverse=True
        )
...
class CitationHistoryTracker:
    ...
    def get_growth_trends(self, repo_name: str) -> Optional[Dict]:
...
def generate_citation_report(repos_data: List[Dict]) -> Dict[str, Any]:
    ...
    print(f"Citation report saved to data/citation_report.json")
    return report
