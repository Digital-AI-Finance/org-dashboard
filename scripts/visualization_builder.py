...
class VisualizationBuilder:
    ...
    def create_publication_timeline(self, repos_data: List[Dict]) -> str:
        ...
        fig = go.Figure(data=[
            go.Bar(x=years, y=counts, marker_color='#1f77b4')
        ])
    ...
    def create_research_impact_chart(self, impact_metrics: Dict) -> str:
        ...
        fig.add_trace(
            go.Bar(x=repos, y=h_indices, name='H-Index', marker_color='#2ca02c'),
            row=1, col=3
        )

        fig.update_layout(
            title_text='Research Impact Metrics',
            showlegend=False,
            height=400
        )
    ...
    def create_collaboration_network(self, repos_data: List[Dict]) -> str:
        ...
        network_data = {
            'nodes': [{'id': repo, 'authors': len(authors)} for repo, authors in repo_authors.items()],
...
def generate_all_visualizations(repos_data: List[Dict], citation_report: Dict = None) -> Dict[str, str]:
    ...
    if citation_report:
        ...
        # Research impact
        impact_path = builder.create_research_impact_chart(citation_report.get('impact_metrics', {}))
        if impact_path:
